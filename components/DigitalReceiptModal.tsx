...
export const DigitalReceiptModal: React.FC<DigitalReceiptModalProps> = ({
  ...
  const handleShareWhatsApp = () => {
    ...
    const message = `🎟️ *COMPROVANTE / BILHETE DIGITAL OFICIAL*\n\n` +
      `📌 *${raffle.title}*\n` +
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
      <div className="bg-white rounded-3xl max-w-md w-full shadow-2xl border-2 border-[#eee4db] overflow-hidden text-[#2d2a26]">
        {/* Header */}
        <div className="bg-[#5A5A40] text-white p-4 flex items-center justify-between border-b border-[#484832]">
        ...
        {/* Ticket Graphic Body in Natural Tones */}
        <div className="p-6 space-y-4">
          <div className="relative rounded-2xl bg-[#fdfaf7] p-5 border-2 border-dashed border-[#eee4db] shadow-inner text-center">
            {/* Stamp */}
            ...
            <h4 className="font-serif font-black text-base text-[#2d2a26] mt-0.5 uppercase">
              {raffle.title}
            </h4>

            {/* Giant Number */}
            <div className="my-4 py-3 bg-[#5A5A40] text-white rounded-2xl border border-[#484832] shadow-inner">
              <span className="text-xs text-[#e6dfd8] block uppercase tracking-widest font-sans font-semibold">
                NÚMERO DA SORTE
              </span>
              <span className="text-5xl font-mono font-black tracking-tight text-[#fdfaf7]">{numFormatted}</span>
            </div>
            ...
            <div className="space-y-1.5 text-xs text-left bg-white p-3 rounded-xl border border-[#eee4db]">
              <div className="flex justify-between">
                ...
                <strong className="text-[#2d2a26]">{numberData.buyerName || 'Sem nome'}</strong>
              </div>
              <div className="flex justify-between">
          ...
          <div className="space-y-2 pt-2">
            {numberData.buyerPhone && (
              <button
                ...
                className="w-full py-3 bg-[#5A5A40] hover:bg-[#484832] text-white font-bold text-xs rounded-xl shadow-xs transition-colors flex items-center justify-center gap-2"
              >
            ...
            <div className="flex gap-2">
              <button
                onClick={handlePrint}
                className="flex-1 py-2.5 bg-[#f8f5f0] hover:bg-[#eee4db] text-[#2d2a26] border border-[#eee4db] font-semibold text-xs rounded-xl transition-colors flex items-center justify-center gap-1.5"
              >
