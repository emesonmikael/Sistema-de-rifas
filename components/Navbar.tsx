      ...
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md text-[#423d38] border-b border-[#eee4db] shadow-xs w-full">
        <div className="max-w-7xl mx-auto px-2 sm:px-4 lg:px-6 h-14 sm:h-16 flex items-center justify-between gap-1.5 sm:gap-3">
          ...
          <div className="relative flex items-center gap-1.5 sm:gap-2.5 shrink-0" ref={dropdownRef}>
            ...
            >
              <div className="min-w-0">
                <div className="flex items-center gap-1">
                  <span className="font-black tracking-tight text-[11px] sm:text-xs text-[#2d2a26] leading-none">
                    RifaPix
                  </span>
                  <span className="text-[9px] bg-[#e8f0e8] text-[#1e7e34] border border-[#c8e6c9] px-1 py-0.2 rounded font-bold">
                    {raffles.length} {raffles.length === 1 ? 'rifa' : 'rifas'}
                  </span>
                </div>
                <div className="flex items-center gap-1 mt-0.5">
                  <p className="text-[10px] sm:text-xs font-semibold text-[#5A5A40] group-hover:text-[#2d2a26] truncate">
                    {activeRaffleTitle}
                  </p>
                  <ChevronDown className={`w-3.5 h-3.5 text-[#5A5A40] shrink-0 transition-transform ${showRaffleDropdown ? 'rotate-180' : ''}`} />
                </div>
              </div>
            </button>
            ...
            {showRaffleDropdown && (
              <div className="absolute top-full left-0 mt-2 w-72 sm:w-84 bg-white rounded-2xl border-2 border-[#eee4db] shadow-2xl p-2.5 z-50 animate-fade-in text-[#2d2a26]">
